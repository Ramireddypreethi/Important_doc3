...
SELECT COUNT(CAR_NAME),car_price FROM CAR GROUP BY CAR_PRICE HAVING CAR_PRICE>12000; -- IF WE WANT TO GET OUTPUT BASED ON CAR_PRICE WE USE "GROUP BY" AND BASED ON CONDITION WE USE "HAVING"
SELECT COUNT(CAR_NAME) FROM CAR GROUP BY CAR_PRICE>22000;-- it grouped as that ,in this there 4 price greater than 22000 and 2 are less than 22000 so it displayed output as 4 and 2.
...
SELECT EXTRACT(YEAR FROM ENTRY_DATE)  AS YEAR,
EXTRACT(MONTH FROM ENTRY_DATE) AS MONTH,
ROUND(AVG(PRICE),2) FROM VISIT;-- doubt
...
SELECT ENTRY_DATE,ROUND(AVG(PRICE),5) AS AVG_PRICE FROM VISIT GROUP BY ENTRY_DATE HAVING COUNT(*) >1 ORDER BY ENTRY_DATE;-- ENTRY_DATE SHOULD BE MORE THAN 1 IN ORDER,HERE ORDER IS TAKEN AS "THE DATE WHICH IS ENTERED FIRST"

SELECT ENTRY_DATE,ROUND(AVG(PRICE),5) AS AVG_PRICE,COUNT(ENTRY_DATE) FROM VISIT GROUP BY ENTRY_DATE HAVING COUNT(*)>1;
SELECT * FROM VISIT;
SELECT COUNT(ENTRY_DATE),ENTRY_DATE FROM VISIT GROUP BY ENTRY_DATE;

-- IF DURATION IS MORE THAN 20 THEN ONLY DO AVG
SELECT ENTRY_DATE,ROUND(AVG(PRICE),2) AS AVG_PRICE FROM VISIT WHERE DURATION >10 GROUP BY ENTRY_DATE HAVING COUNT(*)>0 ORDER BY ENTRY_DATE;
-- ----------------------------------------------------------------------------------------
...
SELECT SUM(SALARY) AS T_SALARY,SALARY FROM MANAGER1 GROUP BY SALARY HAVING SUM(SALARY)>12000 ORDER BY SALARY;
...
END //
CALL GetManagerAge() -- In this first we should execute delimiter to end
                     -- And then should execute call function
